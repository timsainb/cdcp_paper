...
def compute_interpolation_sm_and_neurometric(
    ...
    for interpolation in tqdm(
        ...
        avg_response_vectors, interp_points_this_unit = get_average_response_vector(
            interp_trial_aligned_spikes, use_bins=True
        )
        ...
        # save results
        all_dict["sm_{}".format(interpolation)] = sm
        all_dict["rv_{}".format(interpolation)] = rv
        all_dict["ips_{}".format(interpolation)] = interp_points_this_unit
        all_dict["nm_min_{}".format(interpolation)] = _min
        all_dict["nm_max_{}".format(interpolation)] = _max
        all_dict["nm_r2_{}".format(interpolation)] = r_squared
        all_dict["nm_inflection_{}".format(interpolation)] = _inflection
        all_dict["nm_slope_{}".format(interpolation)] = _slope
        all_dict["nm_range_{}".format(interpolation)] = _max - _min
        all_dict["nm_scaled_slope_{}".format(interpolation)] = _slope / (_max - _min)
    ...
    sm_list = [
        all_dict["sm_{}".format(interpolation)]
        for interpolation in tqdm(
            trial_aligned_spikes.interp.unique(), desc="interpolation", leave=False
        )
        if "sm_{}".format(interpolation) in all_dict.keys()
    ]
    ...
    # save results
    all_dict["sm_{}".format(interpolation)] = sm
    all_dict["rv_{}".format(interpolation)] = avg_response_vectors
    all_dict["ips_{}".format(interpolation)] = interp_points_this_unit
    all_dict["nm_min_{}".format(interpolation)] = _min
    all_dict["nm_max_{}".format(interpolation)] = _max
    all_dict["nm_inflection_{}".format(interpolation)] = _inflection
