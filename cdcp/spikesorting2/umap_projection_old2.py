...
class WaveformParametricUMAP:
    def __init__(
        ...
        if encoder_network is None:
            encoder_network = make_encoder_network(n_components, self.dims)
    ...
    def train(
        ...
        sample_spikes_per_recording = (
            (np.array(n_spikes_per_recording_list) / np.sum(n_spikes_per_recording_list))
            * epoch_size
        ).astype(int)
    ...
    def run_for_extractor(
        ...
        if projection_memmap_file_path is None:
            projection_memmap_file_path = (
                Path(waveform_extractor_path) / "{}{}.npy".format(projection_filename, save_extension)
            )
        else:
        ...
        np.save(
            projection_memmap_file_path.parent / "{}_label{}.npy".format(projection_filename, save_extension), spike_labels
        )
        np.save(
            projection_memmap_file_path.parent / "{}_frame{}.npy".format(projection_filename, save_extension), spike_times
        )
...
def make_encoder_network(n_components, dims):
    encoder = tf.keras.Sequential(
        [
            tf.keras.layers.InputLayer(input_shape=dims),
            tf.keras.layers.Conv2D(
                filters=128,
                kernel_size=(dims[0], 1),
                strides=(1, 1),
                activation="relu",
                padding="valid",
            ),
...
def train_parametric_umap_single_epoch(
    ...
    with tqdm(
        ...
        for (
            ...
            spike_waveforms, spike_labels = sample_spikes(
                waveform_extractor_path,
                best_channels_index=best_channels_index,
    ...
    # make sampled waveforms dense
    n_waveforms_per_recording = make_waveform_array_dense(
        all_spike_waveforms,
        max_channels_per_template,
    ...
    n_waveforms_per_recording = n_waveforms_per_recording.reshape(
        (
            len(n_waveforms_per_recording),
            np.product(n_waveforms_per_recording.shape[1:]),
        )
    )

    z = model.fit_transform(n_waveforms_per_recording)
...
def train_parametric_umap(
    ...
    recording_good_units_list,
    n_spikes_per_recording_list,
    best_channels_index_list=None,
    n_components=2,
    encoder_network=None,
    parametric_umap_kwargs={"min_dist": 0.5, "verbose": True},
    max_channels_per_template=8,
    epoch_size=100000,  # num spikes in epoch
    sample_chunk_size=10000,  # size of chunks to sample from
    # sample from at least this many chunks per epoch per recording
    min_chunks_to_sample=10,
    n_training_epochs=1,
    n_jobs=1,
):

    parametric_umap_kwargs["n_components"] = n_components
    # set recording best channel list as None if it hasn't been computed yet
    if best_channels_index_list is None:
        best_channels_index_list = np.repeat(None, len(waveform_extractor_paths))
    ...
    we = WaveformExtractor.load_from_folder(waveform_extractor_paths[0])
    n_channels = we.recording.get_num_channels()
    dims = [we.nbefore + we.nafter, n_channels, 1]

    # prepare the encoder network if one does not already exist
    if encoder_network is None:
        encoder_network = make_encoder_network(n_components, dims)

    model = ParametricUMAP(encoder=encoder_network, dims=dims, **parametric_umap_kwargs)

    for epoch in tqdm(range(n_training_epochs)):
        train_parametric_umap_single_epoch(
            model=model,
            waveform_extractor_paths=waveform_extractor_paths,
            best_channels_index_list=best_channels_index_list,
